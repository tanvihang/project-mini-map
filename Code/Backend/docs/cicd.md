...
gcloud builds submit --tag gcr.io/minimap-498608/minimap-backend

# 2. Deploy to Cloud Run (traffic auto-routes to new revision)
gcloud run deploy minimap-backend \
  --image gcr.io/minimap-498608/minimap-backend \
  --region us-central1
```
...
steps:
  - name: gcr.io/cloud-builders/docker
    args:
      - build
      - -t
      - gcr.io/$PROJECT_ID/minimap-backend
      - .
...
steps:
  ...
  - name: gcr.io/google.com/cloudsdktool/google-cloud-cli
    ...
    args:
      ...
      - deploy
      - minimap-backend
      - --image=gcr.io/$PROJECT_ID/minimap-backend
      - --region=us-central1
      - --platform=managed
...
```
git push → Cloud Build → Cloud Run → live at minimap-backend-o4bycaxchq-uc.a.run.app
```
...
1. Go to https://console.cloud.google.com/run → **minimap-backend**
2. Tab **Revisions**
...
```bash
gcloud run services update-traffic minimap-backend \
  --region us-central1 \
